...
namespace Statistic {
    ...
    public partial class FormGraphicsSettings : Form {
        ...
        public enum INDEX_COLOR_VAUES {
            ...
            /// </summary>
            UDG, PBR_HOUR_TO_HOUR, DIVIATION, ASKUE, ASKUE_LK_REGULAR, SOTIASSO, REC, TEMP_ASKUTE,
        ...
        public enum TYPE_UPDATEGUI {
            ...
            LINEAR,
            /// <summary>
            /// Признак отображения на стандартных вкладках
            ///  при графической интерпретации значений на интервале час (в разрезе "час - минуты")
            ///  дополнительной линии ПБР-значений
            /// </summary>
            PBR_HOUR_TO_HOUR,
            /// <summary>
            /// Цвет для значений
        ...
        /// <summary>
        /// Закрытое перечисление  MODE_VIEW (ружим отображения значений в обоих интерпретациях)
        /// </summary>
        private enum MODE_VIEW {
        ...
        public FormGraphicsSettings (DelegateIntFunc fUpdate, DelegateFunc fHide, bool bAllowedChangeShema)
        {
            ...
            // отображение ПБР от часа к часу по умолчанию установлено в БД конфигурации
            pbrHourToHour = HStatisticUsers.IsAllowed((int)HStatisticUsers.ID_ALLOWED.PBR_HOUR_TO_HOUR_DEFAULT_VALUE);
            ...
            // доступность элемента управления для изменения признака отображения ПБР от часа к часу по умолчанию установлено в БД конфигурации
            m_cbxPBRHourToHour.Enabled = HStatisticUsers.IsAllowed ((int)HStatisticUsers.ID_ALLOWED.PBR_HOUR_TO_HOUR);

            bool bGroupBoxSourceData = false;                                       //переменной bGroupBoxSourceData присваиваем false
            MODE_VIEW cstGroupBoxSourceData = MODE_VIEW.AISKUE_3_MIN;     //переменной cstGroupBoxSourceData присваиваем константу=1 (AISKUE_3_MIN)
            ...
            //if ((HStatisticUsers.RoleIsAdmin == true) || (HStatisticUsers.RoleIsKomDisp == true))
            {
                ...
                cstGroupBoxSourceData = MODE_VIEW.COSTUMIZE;                   //переменной cstGroupBoxSourceData присваиваем константу=4 (по умолчанию установлен COSTUMIZE)

                //кнопки АИСКУЭ+СОТИАССО и СОТИАССО(3 мин) становятся активными (да вроде все активные..?)
                m_arRbtnSourceData [(int)MODE_VIEW.AISKUE_PLUS_SOTIASSO].Enabled = HStatisticUsers.IsAllowed ((int)HStatisticUsers.ID_ALLOWED.SOURCEDATA_ASKUE_PLUS_SOTIASSO);
                m_arRbtnSourceData [(int)MODE_VIEW.SOTIASSO_3_MIN].Enabled = HStatisticUsers.IsAllowed ((int)HStatisticUsers.ID_ALLOWED.SOURCEDATA_SOTIASSO_3_MIN);
            } else
                ;
        ...
        {
            m_arRbtnSourceData [(int)MODE_VIEW.AISKUE_PLUS_SOTIASSO].Checked = m_markSourceData.IsMarked ((int)(int)MODE_VIEW.AISKUE_PLUS_SOTIASSO);//??
            m_arRbtnSourceData [(int)MODE_VIEW.AISKUE_3_MIN].Checked = m_markSourceData.IsMarked ((int)(int)MODE_VIEW.AISKUE_3_MIN);
            m_arRbtnSourceData [(int)MODE_VIEW.SOTIASSO_3_MIN].Checked = m_markSourceData.IsMarked ((int)(int)MODE_VIEW.SOTIASSO_3_MIN);
            m_arRbtnSourceData [(int)MODE_VIEW.SOTIASSO_1_MIN].Checked = m_markSourceData.IsMarked ((int)(int)MODE_VIEW.SOTIASSO_1_MIN);
            m_arRbtnSourceData [(int)MODE_VIEW.COSTUMIZE].Checked = m_markSourceData.IsMarked ((int)(int)MODE_VIEW.COSTUMIZE);

            //если нажата кнопка "АИСКУЭ+СОТИАССО", то источнику данных присвоить источник  "АИСКИЭ+СОТИАССО"
            if (m_arRbtnSourceData [(int)MODE_VIEW.AISKUE_PLUS_SOTIASSO].Checked == true)
            ...
            //если нажата кнопка "АИСКУЭ(3мин)", то источнику данных присвоить источник  "АИСКУЭ(3мин)"
                if (m_arRbtnSourceData [(int)MODE_VIEW.AISKUE_3_MIN].Checked == true)
                m_connSettType_SourceData = StatisticCommon.CONN_SETT_TYPE.DATA_AISKUE;
            else
                    if (m_arRbtnSourceData [(int)MODE_VIEW.SOTIASSO_3_MIN].Checked == true)           //аналогично
                m_connSettType_SourceData = StatisticCommon.CONN_SETT_TYPE.DATA_SOTIASSO_3_MIN;
            else
                        if (m_arRbtnSourceData [(int)MODE_VIEW.SOTIASSO_1_MIN].Checked == true)
                m_connSettType_SourceData = StatisticCommon.CONN_SETT_TYPE.DATA_SOTIASSO_1_MIN;
            else
                            if (m_arRbtnSourceData [(int)MODE_VIEW.COSTUMIZE].Checked == true)
                m_connSettType_SourceData = StatisticCommon.CONN_SETT_TYPE.COUNT_CONN_SETT_TYPE;
            else
                ;
        }

        private void cbxPBRHourToHour_CheckedChanged (object sender, EventArgs e)
        {
            pbrHourToHour = m_cbxPBRHourToHour.Checked;                               //полю присвоить проверенное  значение
            delegateUpdateActiveGui ((int)TYPE_UPDATEGUI.PBR_HOUR_TO_HOUR);         //обновить активную настройку (масштаб)
        }
        /// <summary>
        ...
        public void SetPBRHourToHour ()
        {
            //инвертировать текущее значение масштаба
            m_cbxPBRHourToHour.Checked = !m_cbxPBRHourToHour.Checked;
        }
        ...
        {
            rbtnSourceData_Click ((MODE_VIEW)(sender as Control).Tag);
        }
        ...
        private void rbtnSourceData_Click (MODE_VIEW indx)
        {
