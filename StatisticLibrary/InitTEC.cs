...
#region Добавить компоненты ТЭЦ (ГТП, ЩУ)
        ...
        public List<TEC> InitTEC(FormChangeMode.MODE_TECCOMPONENT mode, bool bIgnoreTECInUse, int []arTECLimit, bool bUseData) //indx = {GTP или PC}
        {
            ...
            TEC newTECItem;
            List<FormChangeMode.MODE_TECCOMPONENT> modes;
            ...
            if (err == 0)
                for (indx_tec = 0; indx_tec < list_tec.Rows.Count; indx_tec ++) {
                    ...
                    initTECConnectionSettings(newTECItem, list_tec.Rows[indx_tec]);
                    id_tec = Convert.ToInt32 (list_tec.Rows [indx_tec] ["ID"]);
                    // получить список компонентов, с учетом типа компонентов по 'mode'
                    if (mode == FormChangeMode.MODE_TECCOMPONENT.ANY) {
                    //??? что значит < 0, mode.Unknown < 0!!!
                    // все режимы для "электрических"(GTP + PC) компонентов
                        modes = new List<FormChangeMode.MODE_TECCOMPONENT> { FormChangeMode.MODE_TECCOMPONENT.GTP, FormChangeMode.MODE_TECCOMPONENT.PC };
                    } else
                        modes = new List<FormChangeMode.MODE_TECCOMPONENT> { mode };

                    modes.ForEach (m => {
                        list_TECComponents = GetListTECComponent (FormChangeMode.getPrefixMode (m), id_tec, out err);

                        if (err == 0) {
                            initTECComponents (newTECItem, m, list_TECComponents, all_PARAM_DETAIL);
                        } else
                            ; //Ошибка ???
                    });
                }
