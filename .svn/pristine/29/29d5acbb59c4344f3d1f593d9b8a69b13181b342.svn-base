...
namespace trans_tg
{
    public partial class FormMainTransTG : FormMainTrans
    {
        ...
        protected override void setUIControlSourceState(bool bEnabled)
        {
            ...
            TEC tec = ((AdminTS)m_arAdmin [(Int16)StatisticTrans.CONN_SETT_TYPE.DEST]).FindTECComponent (((AdminTS_NSS)m_arAdmin [(Int16)StatisticTrans.CONN_SETT_TYPE.DEST]).m_listKeyTECComponentDetail [0]).tec;

            m_arUIControls [(Int16)StatisticTrans.CONN_SETT_TYPE.SOURCE, (Int16)INDX_UICONTROLS.SERVER_IP].Text =
                tec.GetAddingParameter(TEC.ADDING_PARAM_KEY.PATH_RDG_EXCEL).ToString();
            enabledButtonSourceExport(m_arUIControls[(Int16)StatisticTrans.CONN_SETT_TYPE.SOURCE, (Int16)INDX_UICONTROLS.SERVER_IP].Text.Length > 0 ? true : false);
        }

        private int getIndexGTPOwner(int indx_tg)
        {
            StatisticTrans.CONN_SETT_TYPE indxDB = m_IndexDB;
            int id_gtp_owner = ((DataGridViewAdminNSS)m_dgwAdminTable).GetIdGTPOwner(indx_tg);
        ...
        {
            ((AdminTS_NSS)m_arAdmin[(int)StatisticTrans.CONN_SETT_TYPE.SOURCE]).SaveChanges();
        }

        protected override void SaveRDGValues(bool bCallback)
        {
            ((AdminTS)m_arAdmin[(int)(Int16)StatisticTrans.CONN_SETT_TYPE.DEST]).SaveRDGValues(SelectedItemKey, dateTimePickerMain.Value, bCallback);
        }
        ...
        protected override void setDataGridViewAdmin(DateTime date, bool bNewValues, IList<HAdmin.RDGStruct> values)
        {
            ...
            {
                if (((AdminTS_NSS)m_arAdmin[(int)StatisticTrans.CONN_SETT_TYPE.SOURCE]).CompletedGetRDGValues == true)
                {
                    m_arAdmin[(int)StatisticTrans.CONN_SETT_TYPE.DEST].getCurRDGValues(m_arAdmin[(int)StatisticTrans.CONN_SETT_TYPE.SOURCE]);
