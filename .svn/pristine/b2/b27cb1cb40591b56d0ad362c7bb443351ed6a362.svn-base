    ...
    public abstract class PanelTecViewStandard : PanelTecViewBase
    {
        ...
        public class DataGridViewStandardMins : HDataGridViewStandard
        {
            ...
            public DataGridViewStandardMins()
                ...
                : base(HDateTime.INTERVAL.MINUTES
                        , new ColumnProperies[] {
                            ...
                            , new ColumnProperies (50, 16, $"{HAdmin.PBR_PREFIX}э", @"PBReMin")
            ...
            public override void Fill(TecView.valuesTEC[] values, params object[] pars)
            {
                int hour = (int)pars[0]
                    ...
                    , cnt = -1
                    , iDisplayedRowCount = -1
                    , iFirstDisplayedScrollingRowIndex = -1;
                ...
                try {
                    ...
                    iDisplayedRowCount = DisplayedRowCount (true);
                    //Назначить крайней видимой строкой - крайнюю строку
                    if ((iDisplayedRowCount > 0)
                        && (RowCount > 0))
                        if (!((RowCount - iDisplayedRowCount + 1) < 0))
                            iFirstDisplayedScrollingRowIndex = RowCount - iDisplayedRowCount + 1;
                        else
                    ...
                    if (!(iFirstDisplayedScrollingRowIndex < 0))
                        ...
                        Logging.Logg ().Warning ($@"DataGridViewStandardMins::Fill () - RowCount=[{RowCount}], iDisplayedRowCount=[{iDisplayedRowCount}]...", Logging.INDEX_MESSAGE.NOT_SET);
                } catch (Exception e) {
                    Logging.Logg ().Exception (e, $@"DataGridViewStandardMins::Fill () - RowCount=[{RowCount}], iDisplayedRowCount=[{iDisplayedRowCount}]...", Logging.INDEX_MESSAGE.NOT_SET);
                }
            }
        ...
        protected class HZedGraphControlStandardMins : HZedGraphControl
        {
            ...
            public override void Draw(TecView.valuesTEC[] values, params object[] pars)
            {
                ...
                double [] pbrValues = (double [])pars [7]; //значения ПБР для предыдущего и текущего часов
                double recomendation = (double)pars[8]; //m_tecView.recomendation                
                ...
                double[] valuesRecomendation = new double[itemscount];
                double [] valuesPRB = new double [itemscount];
                double [] valuesUDGe = new double [itemscount];
                double [] valuesFact = new double[itemscount];

                udge = values [1].valuesUDGe; // все значения одинаковые
                if ((pbrValues [0] == double.MinValue)
                    || (pbrValues [1] == double.MinValue))
                    pbrValues [0] =
                    pbrValues [1] =
                        udge;
                else
                ...
                for (int i = 0; i < itemscount; i++)
                {
                    valuesPRB [i] = pbrValues [0] + i * ((pbrValues [1] - pbrValues [0]) / (itemscount - 1));
                    valuesUDGe [i] = udge;
                ...
                #region Определить min/max; наименования для точек по оси времени
                for (int i = 0; i < itemscount; i++)
                {
                    ...
                    if (currHour == true)
                        valuesRecomendation[i] = ((i < (lastMin - 1)) || (!(adminValuesReceived == true))) ? 0 : recomendation;
                    else
                        ;
                    ...
                    if (currHour == true)
                    {
                        if (minimum > valuesRecomendation[i] && valuesRecomendation[i] != 0)
                        {
                            minimum = valuesRecomendation[i];
                            noValues = false;
                        }
                    }
                    else
                    ...
                    if (currHour == true)
                    {
                        if (maximum < valuesRecomendation[i])
                            maximum = valuesRecomendation[i];
                        else
                            ;
                    }
                    else
                ...
                if (((!(udge == pbrValues[0]))
                    || (!(udge == pbrValues [1])))
                    && (FormMain.formGraphicsSettings.pbrHourToHour == true))
                    //LineItem curve3 =
                        GraphPane.AddCurve ("ПБРчч", null, valuesPRB, FormMain.formGraphicsSettings.COLOR (FormGraphicsSettings.INDEX_COLOR_VAUES.PBR_HOUR_TO_HOUR));
                else
                    ;
                //LineItem curve2 =
                    GraphPane.AddCurve("УДГэ", null, valuesUDGe, FormMain.formGraphicsSettings.COLOR(FormGraphicsSettings.INDEX_COLOR_VAUES.UDG));
                ...
                switch (FormMain.formGraphicsSettings.m_graphTypes)
                {
                    case FormGraphicsSettings.GraphTypes.Bar:
                            ...
                            || (currHour == false))
                        {
                            ...
                            GraphPane.AddBar("Мощность", null, valuesFact, colorPCurve);
                            //BarItem
                            GraphPane.AddBar("Рекомендуемая мощность", null, valuesRecomendation, FormMain.formGraphicsSettings.COLOR(FormGraphicsSettings.INDEX_COLOR_VAUES.REC));
                        }
                        else
                        {
                            ...
                            switch (lastMin)
                            {
                                case 0:
                                    ...
                                    if (valuesRecomendation[lastMin] > valuesSOTIASSO[lastMin])
                                        order = true;
                                    else
                                        ;
                                    break;
                                ...
                                default:
                                    try
                                    {
                                        ...
                                        //Порядок "накладывания" значений
                                        if (valuesRecomendation[lastMin - 1] > valuesSOTIASSO[lastMin - 1])
                                            order = true;
                                        else
                                            ;
                                    }
                            ...
                            switch (lastMin)
                            {
                                case 0:
                                    ...
                                    if ((adminValuesReceived == true) && (currHour == true))
                                        for (int i = 0; i < itemscount; i++)
                                            listValuesRec.Add((double)(i + 1), valuesRecomendation[i]);
                                    else
                                        ;
                                    break;
                                default:
