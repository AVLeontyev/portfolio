    ...
    class PanelLKView : PanelTecViewBase
    {
        ...
        protected override void InitializeComponent()
        {
            ...
            if (!(m_label == null))
                ...
                m_label.PerformRestruct(PanelTecViewBase.BANK_DEFAULT.HOUR_TABLE_GRAPH_OPER);
            else {
                //Восстановление события
                OnEventRestruct (new LabelViewProperties (BANK_DEFAULT.HOUR_TABLE_GRAPH_OPER));
            }
        }
        ...
        private class PanelQuickDataLK : HPanelQuickData
        {
            ...
            public override void ShowFactValues()
            {
                int indxStartCommonPVal = m_indxStartCommonFirstValueSeries
                    , lastHour = -1 //m_parent.m_tecView.currHour == true ? m_parent.m_tecView.lastHour + 1 : m_parent.m_tecView.lastHour;
                    ;
                double[] powerLastHourMinutes;
                double powerLastHour = -1F;

                if (Equals(m_parent.m_tecView, null) == false) {
                    lastHour = m_parent.m_tecView.lastHour;
