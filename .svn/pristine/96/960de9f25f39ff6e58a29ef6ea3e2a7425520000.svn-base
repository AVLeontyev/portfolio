...
{    
    public class AdminTS : HAdmin
    {
        ...
        public int GetCurrentHour (DateTime dt, CONN_SETT_TYPE type)
        {
            int iRes = -1;
            ...
            //В настройках указано - учитывать дату/время сервера (НЕ игнорировать)
                offset_days = (dt - serverTime.Date).Days;

                if (offset_days < 0)
                //При указанной дате раньше, чем дата на сервере (задним числом)
                    if ((offset_days == -1)
                        && (serverTime.Hour < 1)) //Исключительная ситуация
                        iRes = m_curRDGValues.Length;
                    else
                        iRes = -1; //Недопустимая ситуация
                ...
                //При текущей дате
                    if (type == CONN_SETT_TYPE.ADMIN)
                        //Возможность изменять рекомендации за тек./час только для админ./знач.
                        iRes = serverTime.Hour == 0 ? serverTime.Hour : serverTime.Hour - 0;
                    else
                        if (type == CONN_SETT_TYPE.PBR)
                            iRes = serverTime.Hour;
                        else
        ...
        {
            Errors resultSaving = Errors.NoError;
            ...
            try {
                ...
                else
                {
                    ...
                    //errorData (-1); // '-1' обработка состояний не была начата
                }
            } catch (Exception e) {
                Logging.Logg ().Exception (e, $@"StatisticCommon.Admin_TS::SaveRDGValues () - ...", Logging.INDEX_MESSAGE.NOT_SET);
            }
        }
        ...
        public FormChangeMode.KeyDevice GetIdOwnerTECComponent (FormChangeMode.MODE_TECCOMPONENT ownerMode, FormChangeMode.KeyDevice key)
        {
            ...
            id_tec = allTECComponents.ToList().Find(comp => comp.m_id == key.Id).tec.m_id;
        ...
        {
            return allTECComponents.ToList().Find (comp => !(comp.m_listMCentreId.IndexOf (id_mc) < 0));
        }
    }
}
