...
namespace StatisticTransModes
{
    public class AdminTS_Modes : AdminTS
    {
        ...
        public override ASUTP.Helper.Errors SaveChanges ()
        {
            ...
            int iStateCompleted = -1;
            ASUTP.Core.DelegateIntFunc fCompleted;
            DelegateFuncInt f;
            ...
            try {
                ...
                if (bResSemaDbAccess == true)
                {
                    ...
                    if (saveResult == ASUTP.Helper.Errors.NoError)
                        fCompleted = saveComplete;
                    else
                        fCompleted = errorData;
                    //??? почему SavePPBRValues, а не SaveAdminValues
                    fCompleted?.Invoke ((int)StatesMachine.SavePPBRValues);
                }
                else {
                ...
                Logging.Logg ().Exception (e, $@"StatisticTransModes.AdminTS_Modes::SaceChanges () - ...", Logging.INDEX_MESSAGE.NOT_SET);
            }
