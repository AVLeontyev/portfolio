...
namespace Statistic
{
        ...
        public class DataGridView_Prop_Text_Check : DataGridView_Prop
        {
            ...
            private void create (DataTable tables)
            {
                ...
                foreach (DataRow r in tables.Rows) {
                    ...
                    if (UNITS.LOGIC.Equ (r ["ID_UNIT"]) == true) /* "8" */ {
                    ...
                    this.Rows [this.Rows.Count - 1].Tag = int.Parse (r ["ID"].ToString().Trim());
                ...
                for (int i = 0; i < this.Rows.Count; i++) {
                    try {
                        if (this.Rows [i].Cells [0] is DataGridViewCheckBoxCell) {
                            value = tables [0].Rows [i] ["VALUE"].ToString ();

                            //TODO: по индексу устанавливать значение нельзя
                            // найти соответствие 'row.Tag' и 'tables [0].Rows [i] ["ID_UNIT"]'

                            if (bool.TryParse (value, out bValue) == true)
                                cellValue = bValue;
                            else if (int.TryParse (value, out iValue) == true)
                                cellValue = iValue == 1 ? true
                                    : iValue == 0 ? false
                                        : false;
                            else
                                cellValue = false;

                            this.Rows [i].Cells [0].Value = cellValue;
                        } else
                            this.Rows [i].Cells [0].Value = tables [0].Rows [i] ["VALUE"];
                    } catch (Exception e) {
                        Logging.Logg ().Exception (e, $@"PanelUser.DataGridView_Prop_Text_Check::Update (id_component={id_component}) - индекс=<{i}>, значение=<{tables [0].Rows [i] ["VALUE"]}>...", Logging.INDEX_MESSAGE.NOT_SET);
                    }
                }
    ...
    public enum UNITS { LOGIC = 8, STRING, INT, FLOAT = 12 }
            ...
            public Dictionary<int, UNIT_VALUE> GetProfileItem
            {
                get
                {
                    ...
                    foreach (DataRow r in ASUTP.Helper.HUsers.GetTableProfileUnits.Rows)
                    {
                        ...
                        {
                            dictRes.Add(id_unit, new UNIT_VALUE() { m_value = unitRows[0][@"VALUE"].ToString().Trim(), m_idType = (UNITS)Enum.Parse (typeof(UNITS), unitRows[0][@"ID_UNIT"].ToString()) });
                        }
                        else
    ...
    {
        public static bool Equ (this UNITS self, string value)
        {
            return self  == (UNITS)Enum.Parse (typeof(UNITS), value.Trim());
        }

        public static bool Equ (this UNITS self, object value)
        {
            return self == (UNITS)Enum.Parse (typeof (UNITS), value.ToString().Trim ());
        }
    }
}
