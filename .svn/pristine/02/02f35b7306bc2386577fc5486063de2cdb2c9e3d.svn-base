...
namespace trans_gtp
{
    public partial class FormMainTransGTP : FormMainTrans
    {
        ...
        protected override void Start()
        {
            ...
            for (i = 0; i < (Int16)StatisticTrans.CONN_SETT_TYPE.COUNT_CONN_SETT_TYPE; i++)
            {
                bool bPPBRSavedValues = false;
                if (i == (Int16)StatisticTrans.CONN_SETT_TYPE.DEST)
                ...
                if ((i == (int)StatisticTrans.CONN_SETT_TYPE.DEST)
                ...
                _client.SetIgnoreDate(i, bIgnoreDateTime);
                //m_arAdmin[i].m_ignore_connsett_data = true; //-> в конструктор

                setUIControlConnectionSettings((StatisticTrans.CONN_SETT_TYPE)i);
                ...
                switch ((StatisticTrans.CONN_SETT_TYPE)i) {
                    case StatisticTrans.CONN_SETT_TYPE.SOURCE:
                        ...
                        DbTSQLConfigDatabase.DbConfig ().Register ();
                        break;
                    case StatisticTrans.CONN_SETT_TYPE.DEST:
                        //??? восстановить исходный источник данных
                        break;
                }
            }

            if (!(i < (Int16)StatisticTrans.CONN_SETT_TYPE.COUNT_CONN_SETT_TYPE))
            {
                start ();
            }
            else
                ...
                for (int j = 0; j < (int)DataGridViewAdminKomDisp.COLUMN_INDEX.TO_ALL; j++)
                {
                    switch (j)
                    {
                        case (int)DataGridViewAdminKomDisp.COLUMN_INDEX.PLAN: // План
                            ...
                            ((AdminTS)m_arAdmin[indxDB]).m_curRDGValues[i].pmin = ((AdminTS)m_arAdmin[(int)StatisticTrans.CONN_SETT_TYPE.SOURCE]).m_curRDGValues[i].pmin;
                            ((AdminTS)m_arAdmin[indxDB]).m_curRDGValues[i].pmax = ((AdminTS)m_arAdmin[(int)StatisticTrans.CONN_SETT_TYPE.SOURCE]).m_curRDGValues[i].pmax;

                            ((AdminTS)m_arAdmin[indxDB]).m_curRDGValues[i].pbr_number = ((AdminTS)m_arAdmin[(int)StatisticTrans.CONN_SETT_TYPE.SOURCE]).m_curRDGValues[i].pbr_number;
                            break;
            ...
            if (IsCanSelectedIndexChanged == true)
            {
                ...
                switch (m_modeTECComponent)
                {
                    case FormChangeMode.MODE_TECCOMPONENT.GTP:
                        _client.GetRDGValues (indexDB, dateTimePickerMain.Value.Date);
                        break;
                ...
                setUIControlConnectionSettings(StatisticTrans.CONN_SETT_TYPE.SOURCE);
                setUIControlConnectionSettings(StatisticTrans.CONN_SETT_TYPE.DEST);
            }
            else
                ;
        }
